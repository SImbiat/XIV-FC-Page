...
trait Parse
{
	...
	$soulcrystall= array();
	preg_match(">Soul of the (.*)<\/h2>", $gearHtml, $soulcrystall);
	$soultemp = preg_replace("/>Soul of the.*/", "", $gearHtml);
	$soucrysicon = array();
	preg_match("/img src=\"(?!.*img src=\")(.*png\?\d{10})\"/", $soultemp, $soucrysicon);
        ...
        foreach($itemsMatch as $match) {
            ...
            // active job
            // TODO multilanguage
            $character->activeJob = $soulcrystall['1'];
            $character->activeJobIcon = $soucrysicon['1'];
            $i++;
        }
